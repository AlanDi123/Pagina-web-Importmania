...
export default function ReferidosClient({ referralCode }: ReferidosClientProps) {
  const [referralUrl, setReferralUrl] = useState<string>(() => {
    if (typeof window !== 'undefined' && referralCode) {
      return `${window.location.origin}/registro?ref=${referralCode}`;
    }
    return '';
  });
  ...
  const handleCopyCode = () => {
    if (!referralCode) {
      toast.error('Iniciá sesión para obtener tu código');
      return;
    }
  ...
  const handleCopyLink = () => {
    if (!referralUrl) {
      toast.error('Iniciá sesión para obtener tu link');
      return;
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {referralCode ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle>Iniciá sesión para ver tu código</CardTitle>
            <CardDescription>
              Creá una cuenta o iniciá sesión para obtener tu código de referido único
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
